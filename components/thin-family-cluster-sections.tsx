          ...
          {clusters.map((cluster) => {
            ...
            return (
              ...
              >
                ...
                {cluster.browseGroups.length > 0 && (
                  <div className="mt-6 grid gap-4 lg:grid-cols-2">
                    {cluster.browseGroups.map((group) => (
                      <div key={group.id} className="rounded-2xl border border-border/70 bg-background/85 p-5">
                        ...
                        <div className="mt-4 flex flex-wrap gap-2">
                          {group.categories.map((category) => (
                            <Link
                              key={`${cluster.id}-${group.id}-${category.id}`}
                              href={`/${category.slug}`}
